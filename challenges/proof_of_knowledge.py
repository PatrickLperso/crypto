...
r = remote('socket.cryptohack.org', 13425, level = 'info')
...
def sigma_protocol(interface, p, q, g, w):
...
if __name__ == "__main__":
    ...
    flag = sigma_protocol(interface_, p, q, g, w)
    assert flag == FLAG

    r.readline()
    interface_ = interface
    flag = sigma_protocol(interface_, p, q, g, w)
    assert flag == "crypto{sigma_protocol_complete!}"