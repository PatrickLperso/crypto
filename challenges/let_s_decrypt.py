...
class Challenge():
    ...
    def challenge(self, your_input):
        if not 'option' in your_input:
            ...
            # e=1
            # bytes_to_long(digest) == calculated_digest
            # bytes_to_long(digest) == pow(SIGNATURE, 1, n)  
            # bytes_to_long(digest) == SIGNATURE

            # bytes_to_long(emsa_pkcs1_v15.encode(msg.encode(), 256)) == SIGNATURE & msg = "I am Mallory[...............]own CryptoHack.org"
            # SHA1()

            if bytes_to_long(digest) == calculated_digest: 
                # emsa_pkcs1_v15.encode( your_input['msg'].encode(), 256) = pow(SIGNATURE,  int(your_input['e'], 16), int(your_input['N'], 16))
...
def recover(interface):
    ...
    msg =      'I am Mallory                        own CryptoHack.org'
    msg_sign = 'We are hyperreality and Jack and we own CryptoHack.org'
    
    N = bytes([0,1]) + bytes([255]) * (256 - 3 - 35) + bytes([0]) + bytes_to_long(hashlib.sha1(msg.encode()).digest()) - bytes_to_long(hashlib.sha1(msg_sign.encode()).digest())

    secret =  interface({'option':'verify', "msg": msg, "N":hex(N), "e":hex(e)})
