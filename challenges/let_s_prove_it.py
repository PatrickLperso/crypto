...
r = remote('socket.cryptohack.org', 13430)
...
def add_random_nonprintable(byte_str):
    # chelou mais en gros ca va placer à un indice aléatoire un byte non printable
...
def xor_nonce(byte_str, nonce):
    # return byte_str[:7] +  xor(middle, nonce) + bytes(end)
    # btw len("crypto{") == 7 
...
class Challenge:
    ...
    def fiatShamir(self):
        ...
        # on génére 512 bits à partirde notre PRNG
