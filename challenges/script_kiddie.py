import os, sys
from Crypto.Cipher import AES
import hashlib
import secrets
from Crypto.Util.number import isPrime
import math 
from deriving_symetric_keys import decrypt_flag 
import gmpy2

sys.path.append(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "general_crypto"))
from Polhig_Hellman_DL import order_g_f
from sage.all import Integer

def header():
    print("""  _____  _  __  __ _
 |  __ \(_)/ _|/ _(_)
 | |  | |_| |_| |_ _  ___
 | |  | | |  _|  _| |/ _ \
 | |__| | | | | | | |  __/
 |_____/|_|_| |_| |_|\___|
 | |  | |    | | |
 | |__| | ___| | |_ __ ___   __ _ _ __
 |  __  |/ _ \ | | '_ ` _ \ / _` | '_ \
 | |  | |  __/ | | | | | | | (_| | | | |
 |_|  |_|\___|_|_|_| |_| |_|\__,_|_| |_|

                                        """)


def is_pkcs7_padded(message):
    padding = message[-message[-1]:]
    return all(padding[i] == len(padding) for i in range(0, len(padding)))


def pkcs7_unpad(message, block_size=16):
    if len(message) == 0:
        raise Exception("The input data must contain at least one byte")
    if not is_pkcs7_padded(message):
        return message
    padding_len = message[-1]
    return message[:-padding_len]


def decrypt_flag(shared_secret: int, iv: str, ciphertext: str):
    # Derive AES key from shared secret
    sha1 = hashlib.sha1()
    sha1.update(str(shared_secret).encode('ascii'))
    key = sha1.digest()[:16]
    # Decrypt flag
    ciphertext = bytes.fromhex(ciphertext)
    iv = bytes.fromhex(iv)
    cipher = AES.new(key, AES.MODE_CBC, iv)
    plaintext = cipher.decrypt(ciphertext)
    return pkcs7_unpad(plaintext).decode('ascii')

def generate_public_int(g, a, p):
    return g ^ a % p # ^  is a XOR


def generate_shared_secret(A, b, p):
    return A ^ b % p


def goodbye():
    print('Goodbye!')


def main():
    header()
    print('[-] Collecting data from Alice')

    # qu'est qui assure que p est prime ?
    p = int(input('> p: '))

    # q fait penser à un safe prime ? Qu'est ce qui assure que q est bien prime ?
    q = (p - 1) // 2

    # Qu'est ce qui assure que g n'est pas dans un sous groupe vulnérable ?
    g = int(input('> g: '))

    # La clé d'Alice 
    A = int(input('> A: '))
    print('[+] All data collected from Alice')

    print('[+] Generating public integer for alice')
    b = secrets.randbelow(q) # génération d'une clé entre [0, q]
    B = generate_public_int(g, b, p)
    print(f'[+] Please send the public integer to Alice: {B}')
    print('')
    input('[+] Press any key to continue')
    print('')

    print('[+] Generating shared secret')
    shared_secret = generate_shared_secret(A, b, p)

    query = input('Would you like to decrypt a message? (y/n)\n')
    if query == 'y':
        iv = input('[-] Please enter iv (hex string)\n')
        ciphertext = input('[-] Please enter ciphertext (hex string)\n')
        flag = decrypt_flag(shared_secret, iv, ciphertext)
        print(f'[+] Flag recovered: {flag}')
        goodbye()
    else:
        goodbye()



""""
p: 
g: 2
A: 
B: 
iv: 'c044059ae57b61821a9090fbdefc63c5'
encrypted_flag: 'f60522a95bde87a9ff00dc2c3d99177019f625f3364188c1058183004506bf96541cf241dad1c0e92535564e537322d7'
"""

if __name__ == '__main__':
    # main()

    # en gros il y a un gars qui a utilisé le programme existant pour chiffrer des trucs 
    # et il faut retourver la clé pour chiffer le bordel

    p = 
    g = 2
    A = 
    B = 
    iv = 'c044059ae57b61821a9090fbdefc63c5'
    encrypted_flag =  'f60522a95bde87a9ff00dc2c3d99177019f625f3364188c1058183004506bf96541cf241dad1c0e92535564e537322d7'

    
    # is it safeprime ?
    assert isPrime(p)
    q = (p - 1) // 2
    assert isPrime(q)

    # g is generator of the safe group of order q (quadratic residu)
    order_prime_decomposition = [(2,1),(q,1)]
    order_prime_decomposition, order_g = order_g_f(g, p, order_prime_decomposition)

    assert order_g == q
    # B = g XOR (B%p) <=> B XOR g = g XOR g XOR (b%p) = 0 XOR (b%p) = (b%p)
    # So  B XOR g = (b%p)
    # Shared_secret = A XOR (B%p) = A XOR B XOR g
    # So 
    shared_secret = A ^ B ^ g
    flag = decrypt_flag(int(shared_secret), iv, encrypted_flag)
    assert flag == 'crypto{b3_c4r3ful_w1th_y0ur_n0tati0n}'
